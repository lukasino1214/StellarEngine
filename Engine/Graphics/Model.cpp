    ...
    void Model::draw(FrameInfo frameInfo, VkPipelineLayout pipelineLayout) {
        for (auto &primitive: primitives) {
            if (hasIndexBuffer) {
                ...
                vkCmdBindDescriptorSets(frameInfo.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0,
                                        sets.size(), sets.data(), 0, nullptr);
                vkCmdDrawIndexed(frameInfo.commandBuffer, primitive.indexCount, 1, primitive.firstIndex,
