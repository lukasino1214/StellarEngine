...
void main()
{
    ...
    switch (push.mip_level)
    {
        ...
        downsample = vec4(groups[0]+groups[1]+groups[2]+groups[3]+groups[4], 1.0);
        downsample = max(downsample, 0.0001f);
        break;
        default:
        downsample  = vec4(e*0.125, 1.0);                // ok
        downsample += vec4((a+c+g+i)*0.03125, 1.0);     // ok
        downsample += vec4((b+d+f+h)*0.0625, 1.0);      // ok
        downsample += vec4((j+k+l+m)*0.125, 1.0);       // ok
        break;
    }
}